...
from lib.generateVideoWithImagecombined import generateVideoWithImages
...
if getattr(gv, 'uploadToYoutube') and gv.uploadToYoutube == True:
    ...
    currentScheduleTime = readPromptFile(gv.youtubeScheduleFile)
    dt = datetime.fromisoformat(currentScheduleTime)
    newScheduleTime  = dt + timedelta(hours=12)
    newScheduleTime = newScheduleTime.isoformat()
    initializeUpload(f"{finalPath}/Videos/final_video_with_subtitles.mp4", storyTitle, videoDescription, thumbnilFile=thumbnilFile, scheduledTime=newScheduleTime)
